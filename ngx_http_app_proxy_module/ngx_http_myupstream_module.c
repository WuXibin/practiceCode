...
static ngx_int_t ngx_http_myupstream_process_status_line(ngx_http_request_t *r);
...
static void ngx_http_myupstream_finalize_request(ngx_http_request_t *r,
    ngx_int_t rc);

static void *ngx_http_myupstream_create_loc_conf(ngx_conf_t *cf);
static char *ngx_http_myupstream_merge_loc_conf(ngx_conf_t *cf,
    void *parent, void *child);

static char *ngx_http_myupstream_pass(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
...
static ngx_command_t  ngx_http_myupstream_commands[] = {
    ...
    { ngx_string("myupstream_connect_timeout"),
    ...
    { ngx_string("myupstream_send_timeout"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_msec_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_myupstream_loc_conf_t, upstream.send_timeout),
      NULL },
    ...
    { ngx_string("myupstream_read_timeout"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_msec_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_myupstream_loc_conf_t, upstream.read_timeout),
      NULL },
...
ngx_http_myupstream_handler(ngx_http_request_t *r)
{
    ...
    ngx_str_set(&u->schema, "myupstream://");
    u->output.tag = (ngx_buf_tag_t) &ngx_http_myupstream_module;

    mlcf = ngx_http_get_module_loc_conf(r, ngx_http_myupstream_module);

    u->conf = &mlcf->upstream;

    u->create_request = ngx_http_myupstream_create_request;
    u->reinit_request = ngx_http_myupstream_reinit_request;
    u->process_header = ngx_http_myupstream_process_status_line;
    u->abort_request = ngx_http_myupstream_abort_request;
    u->finalize_request = ngx_http_myupstream_finalize_request;

    ctx = ngx_palloc(r->pool, sizeof(ngx_http_myupstream_ctx_t));
    if (ctx == NULL) {
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
    }
    ...
    ngx_http_set_ctx(r, ctx, ngx_http_myupstream_module);

    /*
    u->input_filter_init = ngx_http_myupstream_filter_init;
    u->input_filter = ngx_http_myupstream_filter;
    ...
    */
    ...
    struct hostent *pHost = gethostbyname((char*) "33.autohome.com.cn");
    if (pHost == NULL) {
...
ngx_http_myupstream_create_request(ngx_http_request_t *r)
{
    ...
    ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0, 
            "uri: %V\targs: %V", &r->uri, &r->args);
    static ngx_str_t backendQueryLine = ngx_string("GET %V?%V HTTP/1.1\r\nHost: 33.autohome.com.cn\r\nConnection: close\r\n\r\n");

    len = backendQueryLine.len + r->uri.len + r->args.len - 4;
    ...
    ngx_snprintf(b->pos, len, (char*)backendQueryLine.data, &r->uri, &r->args);
    b->last = b->pos + len;
...
static ngx_int_t
ngx_http_myupstream_reinit_request(ngx_http_request_t *r)
{
    return NGX_OK;
}
...
static ngx_int_t
ngx_http_myupstream_process_status_line(ngx_http_request_t *r)
{
    size_t                 len;
    ngx_int_t              rc;
    ngx_http_upstream_t   *u;
    ...
    if (rc == NGX_ERROR) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "upstream sent no valid HTTP/1.1 header");
    ...
    //myupstream_upstream_process_header，
    ...
    //将由myupstream_upstream_process_header方法解析
    return ngx_http_myupstream_process_header(r);
}
...
{
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "finalize http myupstream request");
    return;
}
...
static char *
ngx_http_myupstream_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    u.url = value[1];
    ngx_log_error(NGX_LOG_DEBUG, cf->log, 0, "upstream host: %V", &value[1]);
    u.no_resolve = 1;
    ...
    ngx_http_upstream_srv_conf_t *upstream = mlcf->upstream.upstream;
    ngx_log_error(NGX_LOG_DEBUG, cf->log, 0, "local host: %V", &upstream->host);
