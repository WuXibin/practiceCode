...
static ngx_int_t ngx_http_plugin_init_process(ngx_cycle_t *cycle) {
    ...
    ngx_log_error(NGX_LOG_DEBUG, cycle->log, 0, "[plugin] init process start");

    if(plugin_create_manager(conf) != NGX_OK) {
        ngx_log_error(NGX_LOG_ERR, cycle->log, 0, "[plugin] init process fail");
        return NGX_ERROR;   
    }

    ngx_log_error(NGX_LOG_DEBUG, cycle->log, 0, "[plugin] init process success");
