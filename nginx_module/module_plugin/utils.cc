...
static void split_string(map<string, string> &kv, const string &str, char delimiter);
...
int ngxr_to_crequest(void *ngxr, void *creq) {
    ...
    if(r->args.len > 0) {
        ...
        split_string(request->param, uri_dec, '&');
    }
    ...
    for(size_t i = 0; i < r->headers_in.cookies.nelts; ++i) {
        ...
        split_string(request->headers_in.cookies, cookie_str, ';'); 
    }
...
static void split_string(map<string, string> &kv, 
    ...
    char *beg = (char *)cstr, *end = beg + str.length();
