...
ngx_int_t ngx_http_adprotocol_encode(ngx_str_t * data, ngx_pool_t * pool, ngx_buf_t ** buf)
{
    ...
    *buf = ngx_create_temp_buf(pool, len);
    ...
    ngx_http_adprotocol_head_t head;
...
ngx_int_t ngx_http_adprotocol_decode(ngx_buf_t * buf)
{
    if (buf == NULL) {
        return NGX_ERROR;
    }
    ...
    ngx_http_adprotocol_head_t * pstHead = (ngx_http_adprotocol_head_t*)buf->pos;
