...
ngx_http_mysubrequest_handler(ngx_http_request_t *r) {
    ...
    if(ctx == NULL) {
        ...
        //ngx_str_t location1 = ngx_string("/myupstream");
        ngx_str_t location1 = ngx_string("/adfront/deliver");
        ...
        ngx_str_t location2 = ngx_string("/myupstream");
        ...
        printf("1r->main->count: %d\n", r->main->count);

        r->main->count++;
        return NGX_DONE;
    }

    printf("2r->main->count: %d\n", r->main->count);
...
ngx_http_mysubrequest_post_handler(ngx_http_request_t *r, void *data, ngx_int_t rc) {
    printf("%s ngx_http_mysubrequest_post_handler\n", r->uri.data);
